    ...
    <div class="row">
        ...
        <div class="col-lg-10" style="margin-top: 50px">
            ...
            <asp:SqlDataSource ID="SqlDataSource1" runat="server" ConnectionString="<%$ ConnectionStrings:ShoppingConnectionString %>" 
                DeleteCommand="DELETE FROM [cart_product] WHERE [o_id] = @o_id" 
                InsertCommand="INSERT INTO [cart_product] ([user_id], [product_id], [Product_name], [brand], [image], [quantity], [price]) VALUES (@user_id, @product_id, @Product_name, @brand, @image, @quantity, @price)" 
                SelectCommand="SELECT * FROM [cart_product] WHERE ([user_id] = @user_id)" 
                UpdateCommand="UPDATE [cart_product] SET [user_id] = @user_id, [product_id] = @product_id, [Product_name] = @Product_name, [brand] = @brand, [image] = @image, [quantity] = @quantity, [price] = @price WHERE [o_id] = @o_id">
                <DeleteParameters>
